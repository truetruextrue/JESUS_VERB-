...
# Processa mídias: renomeia, move para pasta correta e atualiza metadados
...
DESTINO_MAP = {
    ...
    "audio": {
        "NOVA": f"{MIDIA_PATH}/02_AUDIO/04_INFODOSES/",  # NOVA → Infodoses
...
def carregar_catalogo():
    """Carrega o catálogo JSON existente ou cria um novo"""
...
def salvar_catalogo(catalogo):
    ...
    with open(f"{METADATA_PATH}/catalogo_geral.json", 'w', encoding='utf-8') as f:
...
def print_assinatura():
    print("\n" + "="*70)
    print("  🧿 KOBLLUX · MEDIA PROCESSOR 🧿")
    print("  Versão: 3.0 · Frequência: 777Hz · Opcode: 0×07")
    print("  Assinatura: 0x012123456789ABC")
    print("  Funções: Renomear + Mover + Metadados")
    print("="*70 + "\n")
...
def main():
    ...
    print(f"📊 Catálogo atualizado: {METADATA_PATH}/catalogo_geral.json")
    print(f"   Total de mídias: {catalogo['total_midias']}")
    print(f"   Por tipo: Vídeo {catalogo['por_tipo'].get('video',0)} | Áudio {catalogo['por_tipo'].get('audio',0)}")
